...
def square_around_snake_head(size:int, snake_game:SnakeV2.SnakeGame) -> list:
...
class SnakePopulation(neat.Population):
    class SnakeGenome(neat.DefaultGenome):
        def __init__(self, key:int, config:neat.Config=None):
            super().__init__()
            self.network:neat.nn.FeedForwardNetwork = None
            if config:
                self.network = neat.nn.FeedForwardNetwork.create(self, config)
            self.apples_position_history:list[tuple[int,int]] = []
            self.move_history:list[str] = []

        def save(self, file_name:str=None, path:str=None) -> None:
            if not file_name:
                file_name = "SnakeNEAT_genome"
    ...
    def __init__(self, config:neat.Config=None) -> None:
        if config:
            super().__init__(config)
        self.best_fitness:int = -100
        self.average_fitness:int = 0
        self.best_fitness_history:list[int] = []
        self.input_config:list = [None, None, None, None]
        self.output_config:str = ""
        self.best_genome:self.SnakeGenome = self.SnakeGenome(self.config)

    def save(self, file_name:str=None, path:str=None) -> None:
        if not file_name:
            file_name = f"SnakeNEAT_population_{self.generation}generations"
    ...
    def load(self, file_name:str, path:str=None) -> None:
        if ".pkl" not in file_name:
            print(f"{file_name} is not a .pkl")
            return None
...
class SnakeNEAT:
    ...
    def __init__(self, config_path:str=None, config:neat.Config=None) -> None:
        self.snake_population = SnakePopulation()
        self.new_population()
    ...
    def draw_window(self, snake_game:Snake.SnakeGame, fitness_score:int) -> None:
        snake_game.draw_window()
        gen_text = Snake.FONT.render(f"Gen: {str(self.population.generation)}", 1, (255,255,255))
        snake_game.window.blit(gen_text, (Snake.WINDOW_WIDTH - gen_text.get_width(), 0))
        fitness_text = pygame.font.SysFont("comicsans", 16).render(f"Fitness Score: {round(fitness_score, 2)}", 1, (255,255,255))
        snake_game.window.blit(fitness_text, (Snake.WINDOW_WIDTH/2 - fitness_text.get_width()/2, 0))
        best_fitness_text = pygame.font.SysFont("comicsans", 16).render(f"Best Fitness: {round(self.best_fitness, 3)}", 1, (255,255,255))
        snake_game.window.blit(best_fitness_text, (Snake.WINDOW_WIDTH/2 - best_fitness_text.get_width()/2, fitness_text.get_height()))
    ...
    def draw_loading(self, snake_game:Snake.SnakeGame, total_population_size:int, remaining_population_size:int) -> None:
        ...
        snake_game.window.blit(loading_text, (Snake.WINDOW_WIDTH/2 - loading_text.get_width()/2, Snake.WINDOW_HEIGHT/2 - loading_text.get_height()/2))
        pygame.display.update()
